...
public class ChunkRenderer extends IWorldRenderer implements EventListener {
    ...
    public ChunkRenderer(IWorld w, Player p, Camera c, GameSetting setting) {
        super(w, p, c, setting);
        if (this.setting.getValueAsBoolean("client.render.terrain.async_chunk_compile", false)) {
            this.updateService = new AsyncRenderCompileService<>();
        } else {
            this.updateService = new MultiRenderCompileService<>("cubecraft:client_chunk", this.setting.getValueAsInt("client.render.chunk.drawThread", 1));
        }
    ...
    private void drawChunks() {
        ...
        if (this.setting.getValueAsBoolean("client.render.fog", true)) {
    ...
    private void updateChunks() {
        this.updateQueue.removeIf(
                c -> !this.camera.objectDistanceSmallerThan(new Vector3d(c.x() * 16, c.y() * 16, c.z() * 16), this.setting.getValueAsInt("client.render.terrain.renderDistance", 4) * 16)
                        && !this.frustum.aabbVisible(RenderChunk.getAABBFromPos(c, this.camera))
        );

        int maxUPD = this.setting.getValueAsInt("client.render.chunk.maxUpdate", 2);
