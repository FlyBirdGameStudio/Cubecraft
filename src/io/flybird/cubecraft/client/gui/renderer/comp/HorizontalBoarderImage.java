...
public record HorizontalBoarderImage(double x0,double x1,double y0,double y1,int boarder,String loc) implements ComponentPartRenderer {
    ...
    public void render(Node node) {
        ...
        Texture2D tex= Registry.getTextureManager().getTexture2DContainer().get(ResourceLocation.uiTexture(this.loc.split(":")[0],this.loc.split(":")[1]).format());
        double tbh=(double) boarder/ tex.getWidth();

        int x0In=x+boarder,x1In=x+w-boarder,x0Out=x,x1Out=x+w;
        VertexArrayBuilder builder=new VertexArrayBuilder(12);
        builder.begin();
        ShapeRenderer.drawRectUV(builder,x0Out,x0In,y,y+h,z,z,0,tbh,0,1);
        ShapeRenderer.drawRectUV(builder,x0In,x1In,y,y+h,z,z,tbh,1-tbh,0,1);
        ShapeRenderer.drawRectUV(builder,x1In,x1Out,y,y+h,z,z,1-tbh,1,0,1);
        builder.end();
