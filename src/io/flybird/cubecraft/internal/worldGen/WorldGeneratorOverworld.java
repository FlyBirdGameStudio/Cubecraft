...
public class WorldGeneratorOverworld implements IChunkGenerator {
    ...
    public void generateChunkBase(Chunk chunk, WorldGeneratorSetting setting){
        ...
        this.altitudeFinal =new Select(altitudeHigh, altitudeLow,new Scale(new PerlinNoise(new Random(setting.seed()%738210^1327899),4),32d,32d));
    }
        ...
        for (int x=0;x<16;x++){
            for (int z=0;z<16;z++){
                for (int y=0;y<Chunk.HEIGHT;y++){
                    ...
                    chunk.setBiome(x,y,z,Registry.getBiomeMap().match(
    ...
    public void generateChunkTerrain(Chunk chunk,WorldGeneratorSetting setting){
        ...
        for (int x=0;x<16;x++){
            for (int z=0;z<16;z++){
                 ...
                 */
                for (int y=0;y<Chunk.HEIGHT;y++){
                    if(chunk.getKey().toWorldPosY(y)<=0) {
