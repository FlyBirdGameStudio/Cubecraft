...
public abstract class Block {
    public Block(String defaultRegisterId) {
        Registry.getBlockRendererMap().registerItem(defaultRegisterId, new BlockModelRenderer(ResourceLocation.blockModel(defaultRegisterId.split(":")[0], defaultRegisterId.split(":")[1] + ".json")));
    }
...
//  ------ general getter ------
    ...
    public void onInteract(Entity from, IWorld world, long x, long y, long z, byte f) {
        ...
        if (world.isfree(world.getBlockState(pos.x(), pos.y(), pos.z()).getCollisionBox(pos.x(), pos.y(), pos.z()))) {
            world.setBlockState(pos.x(), pos.y(), pos.z(), Registry.getBlockMap().get(from.getSelectBlock()).defaultState());
        }
    }

    public void onHit(Entity from, IWorld world, long x, long y, long z, byte f) {
        world.setBlockState(x, y, z, Registry.getBlockMap().get("cubecraft:air").defaultState());
    }
