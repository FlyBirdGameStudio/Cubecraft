...
//modified fron org.joml,special thanks!
//decompile by fern flower
public class RayInterSection {
    ...
    public RayInterSection(double originX, double originY, double originZ, double dirX, double dirY, double dirZ) {
        this.set(originX, originY, originZ, dirX, dirY, dirZ);
    }

    public RayInterSection(Vector3d cameraPosition, Vector3d to) {
        this(cameraPosition.x,cameraPosition.y, cameraPosition.z,to.x,to.y,to.z);
    }
    ...
    private static long signum(double f) {
        return f != 0.0F && !Double.isNaN(f) ? (1 - Double.doubleToLongBits(f) >>> 31 << 1) - 1 : 0;
    }
    ...
    private void precomputeSlope() {
        ...
        long sgnX = signum(this.dirX);
        long sgnY = signum(this.dirY);
        long sgnZ = signum(this.dirZ);
    ...
    public boolean test(AABB aabb){
        return test(aabb.x0,aabb.y0,aabb.z0,aabb.x1,aabb.y1,aabb.z1);
    }
