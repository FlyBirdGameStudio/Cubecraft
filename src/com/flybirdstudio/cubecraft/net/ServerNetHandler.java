...
public class ServerNetHandler extends INetHandler{
    ...
    @EventHandler
    public void onPacketBlockChange(ClientPacketBlockChange packet){
        ...
        if(new Vector3d(packet.x(),packet.y(),packet.z()).distance(new Vector3d(e.x,e.y,e.z))>e.getReachDistance()||e==null){
            this.send(new ServerPacketBlockChange(packet.x(),packet.y(),packet.z(),world.getBlockState(packet.x(),packet.y(),packet.z())));
        }
        this.world.setBlockState(packet.x(), packet.y(), packet.z(),packet.state());
    }

    @EventHandler
    public void onPacketAttack(ClientPacketAttack packet){
    ...
    @EventHandler
    public void onPacketPositionChange(ClientPacketEntityPositionUpdate packet){
        Entity e=this.world.getEntity(packet.uuid());
        e.setPos(packet.newLoc().x,packet.newLoc().y,packet.newLoc().z);
    }

    @EventHandler
    public void onPacketChunkLoadRequest(ClientPacketChunkLoadRequest packet){
        Entity e=this.world.getEntity(packet.uuid());
    ...
    @EventHandler
    public void onPacketChunkGetRequest(ClientPacketChunkGetRequest packet){
        Entity e=this.world.getEntity(packet.uuid());
        if(new Vector3d(e.x,e.y,e.z).distance(new Vector3d(packet.cx()*16,packet.cy()*16,packet.cz()*16))<512){
            this.world.loadChunk(packet.cx(), packet.cy(), packet.cz(),new ChunkLoadTicket(ChunkLoadLevel.None_TICKING,10));
            this.send(new ServerPacketChunkGetResponse(this.world.getChunk(packet.cx(),packet.cy(),packet.cz())));
        }
    }
