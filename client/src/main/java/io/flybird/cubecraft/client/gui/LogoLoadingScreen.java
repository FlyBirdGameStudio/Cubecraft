...
public class LogoLoadingScreen extends Screen {
    ...
    public void init() {
        ...
        logoTex.load(ClientRegistries.RESOURCE_MANAGER.getResource(ResourceLocation.uiTexture("cubecraft","icons/logo.png")));
    }
    ...
    public void render(DisplayScreenInfo info, float interpolationTime) {
        if(alpha>0){
            ...
            ShapeRenderer.drawRectUV(builder,xc - 160, xc + 160, yc - 50, yc + 30, 1, 0, 1, 0, 1);
            builder.end();
    ...
    public void drawProgressBar(VertexArrayBuilder builder,int xc, int yc,int halfSize, float prog) {
        //render progress bar
        float x0_out = xc - halfSize, x1_out = xc + halfSize, y1_out = yc + 10;
        ...
        builder.vertex(x1_out, (float) yc, 1);
        builder.vertex(x0_out, (float) yc, 1);
