...
public class ClientWorld extends IWorld {
    ...
    public void loadChunk(ChunkPos p, ChunkLoadTicket ticket) {
        this.getEventBus().callEvent(new ChunkLoadEvent(client.getPlayer(), p, ticket));
    }
    ...
    public void tick() {
        ...
        while (it2.hasNext()) {
            ...
            if (e == this.client.getPlayer()) {
                e.tick();
                this.loadChunkRange((long) (e.x) / 16, (long) (e.z) / 16, client.getGameSetting().getValueAsInt("client.world.simulation_distance", 3), new ChunkLoadTicket(ChunkLoadLevel.Entity_TICKING, 10));
            } else {
                Chunk c = this.getChunk(new ChunkPos((long) (e.x) / 16, (long) (e.z) / 16));
